...
The fresh PR 7 pre-change production-only audit reproduced 32 findings (1 low, 12 moderate, 16 high, and 3 critical). The critical installed paths were `@clerk/nextjs@6.36.10`/`@clerk/shared@3.43.2` and `protobufjs@7.5.4` through `@google-cloud/kms@5.4.0 -> google-gax@5.0.6` and `@google/genai`. Targeted upgrades now resolve Clerk to 6.39.6 with shared 3.47.8 and every installed protobuf path to 7.6.3. The final production-only audit reports 25 findings (1 low, 11 moderate, 13 high, zero critical). No automatic audit fix or blanket upgrade was applied; Security owns the documented non-critical triage, and any critical recurrence blocks activation.
...
Acceptance: the threat model is documented, dependency/privacy/policy findings have owners and dispositions, no unsafe obsolete runtime path remains active, and a clean install/build/test plus deployment-tool dry run succeeds. Production deployment remains separately approved work.
...
## PR 7 cleanup and final review

PR 7 centralizes exact canary-scope parsing, four-control startup validation, bounded authenticated operational HTTP, bridge start/fence calls, process-fenced drain/resume sequencing, and operational redaction. Gateway and bridge scripts retain process composition while assignment, gateway authorization, leases, runtime ownership, readiness, rate limiting, and durable auth continue to use their existing authority modules. Missing bridge credentials no longer authorize internal operations; arbitrary bridge exceptions are reduced to allowlisted codes; health emits one-way tenant/session references and a profile-path fingerprint instead of raw identifiers/paths. Operator unbinding no longer switches a device-egress session to server egress.

Final verification: 156/156 focused device-tunnel, rate-limit, session-auth, bridge, ingress/history/media, canary, drain, failure-policy, reassignment, retention, rollback, redaction, and compatibility tests pass. Clerk bearer authorization and PM2 singleton suites pass 2/2 each. Android `testDebugUnitTest` and `assembleDebug`, Prisma validate/generate, targeted strict checks, deployment shell syntax, static/template/dry-run operator tooling, production-style gateway/bridge/API bundles, transformed page-closure inspection, `git diff --check`, a clean `npm ci --legacy-peer-deps`, and the production build pass. The build continues to skip repository-wide type validation and emits the existing dynamic protobuf import warning; full-repository `tsc --noEmit` is not claimed as passing.

Compatibility remains deliberately narrow: controls unset/off use the global URL, local profile, no authoritative lease, and disabled rate limiting; a non-selected binding in a canary-capable process remains compatibility-only. LocalAuth/global-URL/schema cleanup is postponed until every binding is migrated, telemetry proves zero compatibility use for a full release window, rollback is rehearsed without the old path, and a separate reversible change is explicitly approved.

The targeted dependency update resolves the three critical findings: `@clerk/nextjs` resolves to 6.39.6 with backend 2.33.6, React 5.61.9, and shared 3.47.8; every KMS/GenAI protobuf path is pinned to 7.6.3. The post-change production audit reports zero critical findings; remaining findings require Security triage and are not accepted by this document.

See [the PR 7 security, privacy, and policy review](./device-egress-security-review.md) for the startup matrix, trust boundaries, data inventory, compatibility removal criteria, policy sources, owners, and activation blockers. The PR 7 review itself performed no production mutation; a later approved compatibility deployment applied its migration with all controls off.
