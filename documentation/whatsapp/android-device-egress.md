...
- Cross-node browser failover is not active yet: PR 5 code is complete, but production session authentication is still stored on the local host and the durable mode/migration are not deployed. Do not enable multi-node automatic failover, move/copy a live profile, or provision a second node before the PR 6 canary is explicitly approved.
