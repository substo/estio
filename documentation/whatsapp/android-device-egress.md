...
- Private R2 bucket `estio-whatsapp-session-auth-prod` is provisioned with bucket-only item read/write credentials. The application adapter passed immutable PUT, read-back, delete, and post-delete verification; runtime credential material remains outside Git with owner-only permissions.
- PR 6 operations remain inactive. The second Hetzner node and DNS/TLS, per-node workload credential delivery, an exact expiring canary scope, and owner dispositions are still required before activation.
...
Automatic attach has a 120-second database deadline; a stopped-browser checkpoint/detach has a 180-second deadline. Android reconnect retains its existing full-jitter backoff capped at 60 seconds, and deployment allows up to 300 seconds for Android reconnect, encrypted restore, browser collection probing, and authenticated webhook freshness to complete. `starting` and `authenticated` are automatic recovery states: operators must not ask a user to restart Android or scan a QR merely because this bounded recovery is still running. Relinking is appropriate only for an explicit `qr`/`relink_required` outcome after durable recovery has failed. These are fail-closed bounds, not an availability promise: provider or WhatsApp outages leave the session non-ready without switching to server egress.
...
- Cross-node browser failover is not active yet: PR 5 code, its migration, KMS, and private R2 are ready, but production session authentication is still stored on the local host and durable mode is disabled. Do not enable multi-node automatic failover or move/copy a live profile before node-2 DNS/TLS, per-node workload authentication, and an exact canary approval pass the runbook.
