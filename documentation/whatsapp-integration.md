...
| **Queued messages leak into other chats when switching rapidly** | **Issue**: React effect synced old un-updated messages state with the new `activeId`. **Fix**: `syncPendingMessagesForConversation(...)` now explicitly verifies `message.conversationId === activeId` before merging optimistic state. |
...
| **Queued messages never dispatch** | **Issue**: Redis was down or enqueue failed, but the fallback inline-dispatch was skipped because `enqueueDelayMs > 0` due to simulated typing. **Fix**: The system now forcefully bypasses typing delays and dispatches inline immediately if the queue degrades. Check `pm2 logs` for "Queue enqueue degraded; bypassed typing delay and dispatched immediately". Otherwise check the cron fallback (`/api/cron/whatsapp-outbound`). |
