...
| File | Role |
|------|------|
| `middleware.ts` | Detects auth intent and sets `x-enable-satellite` header |
| `app/(public-site)/[domain]/layout.tsx` | Conditionally enables `ClerkProvider` satellite mode |
| `lib/auth/clerk-domains.ts` | `registerClerkDomain()`, `whitelistClerkDomain()` functions |
| `scripts/register-domain.ts` | CLI tool to register a new satellite domain |
