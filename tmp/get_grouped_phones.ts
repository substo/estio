...
async function main() {
  ...
  for (const c of contacts) {
    ...
    const cc = getCountryCode(c.phone);
    if (cc === 'Unknown') continue; 
    
    const key = `${c.locationId}_${cc}`;
    if (!map.has(key)) {
      map.set(key, {
  ...
  if (result.length > 0) {
    const fs = require('fs');
    fs.writeFileSync('tmp/phones_by_country.json', JSON.stringify(result, null, 2));
    console.log("Saved to tmp/phones_by_country.json");
  } else {
