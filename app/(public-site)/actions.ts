...
export async function submitLeadInquiry(
    ...
    try {
        ...
        if (contact) {
            ...
            // Only update phone/name if not present or maybe overwrite? 
            // Let's safe-update: only if missing.
            await db.contact.update({
                where: { id: contact.id },
                data: {
                    phone: contact.phone ? undefined : phone, // Don't overwrite existing phone
        ...
        const inquiryContent = `Inquiry submitted for property ID: ${propertyId}. User Message: ${message || "Interested in viewing."}`;
