...
export function NewConversationDialog({ open, onOpenChange, onConversationCreated, locationId }: NewConversationDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className={cn("flex max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] flex-col gap-3 overflow-hidden p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:max-w-lg sm:gap-4 sm:p-6 sm:pb-6 transition-all duration-300",
                ...
                <Tabs defaultValue="phone" onValueChange={handleTabChange} className="flex min-h-0 flex-1 flex-col">
                    <div className="-mx-1 shrink-0 overflow-x-auto px-1">
                        <TabsList className="w-max min-w-full justify-start">
                            <TabsTrigger value="phone" className="flex-none gap-1.5 px-2.5 text-xs sm:flex-1 sm:text-sm">
                                <Phone className="w-3.5 h-3.5" /> Phone
                            </TabsTrigger>
                            <TabsTrigger value="pick" className="flex-none gap-1.5 px-2.5 text-xs sm:flex-1 sm:text-sm">
                                <Users className="w-3.5 h-3.5" /> WhatsApp
                            </TabsTrigger>
                            {locationId && (
                                <TabsTrigger value="google" className="flex-none gap-1.5 px-2.5 text-xs sm:flex-1 sm:text-sm">
                            ...
                            <TabsTrigger value="paste" className="flex-none gap-1.5 px-2.5 text-xs sm:flex-1 sm:text-sm">
                    ...
                    <TabsContent value="pick" className="mt-4 min-h-0 overflow-y-auto pr-1">
                        <div className="space-y-3">
                            ...
                            {/* Chat List */}
                            <div className="max-h-[min(320px,42dvh)] overflow-y-auto border rounded-lg divide-y">
                                {loadingChats && (
                    ...
                    {locationId && (
                        <TabsContent value="google" className="mt-4 min-h-0 overflow-y-auto pr-1">
                            <div className="space-y-3">
                                {googleAuthExpired && (
                                ...
                                <div className="max-h-[min(320px,42dvh)] overflow-y-auto border rounded-lg divide-y">
                                    ...
                                    {!loadingGoogle && googleResults.map((contact) => {
                                        ...
                                        return (
                                            <div key={contact.resourceName} className="w-full flex flex-col gap-3 p-3 hover:bg-slate-50 transition-colors sm:flex-row sm:items-center sm:justify-between">
                                                <div className="flex w-full min-w-0 items-center gap-3 overflow-hidden">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <div className="flex min-w-0 flex-col gap-0.5 text-xs text-muted-foreground sm:flex-row sm:gap-2">
                                                            {contact.email && <span className="truncate">{contact.email}</span>}
                                                            {contact.phone && <span className="truncate">{contact.phone}</span>}
                                                            {!contact.email && !contact.phone && <span className="truncate">No phone or email</span>}
                                                        </div>
                                                        {rowOutcomeLabel && (
                                                            <div className={cn(
                    ...
                    {/* Tab 3: Paste Lead */}
                    <TabsContent value="paste" className="mt-4 min-h-0 space-y-4 overflow-y-auto pr-1">
                        {!parsedLead ? (
                            <div className="space-y-3">
                                ...
                                <Textarea
                                    ...
                                    className="max-h-[min(240px,32dvh)] min-h-[120px] font-mono text-sm sm:min-h-[150px]"
                                    value={leadText}
                                    onChange={(e) => setLeadText(e.target.value)}
                                    onPaste={handleLeadTextareaPaste}
                                    disabled={isAnalyzing}
                                />
                                <div className="flex flex-col gap-2 text-xs text-gray-500 px-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                <DialogFooter className="shrink-0">
                    <Button variant="outline" onClick={handleClose} disabled={creating} className="w-full sm:w-auto">
                        Cancel
                    </Button>
                </DialogFooter>
            </DialogContent>
