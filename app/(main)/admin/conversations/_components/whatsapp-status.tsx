    ...
    return (
        <div className="flex items-center gap-2 text-xs px-2 py-1 bg-slate-50 border-b">
            ...
            <span className="text-gray-500 font-medium">
                {isConnected ? 'WhatsApp Online' : status === 'checking' ? 'Checking...' : 'WhatsApp Offline'}
            </span>

            <div className="flex-1" />

            {/* Refresh Button */}
            <Button variant="ghost" size="icon" className="h-4 w-4 text-gray-400 hover:text-gray-600" onClick={checkStatus} disabled={loading}>
                ...
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    <DialogTrigger asChild>
                        <Button
                            variant="outline"
                            size="sm"
                            className="h-5 text-[10px] px-2 gap-1 border-red-200 text-red-600 hover:bg-red-50"
                            onClick={() => {
