...
import { updateFavoritesConfig } from "@/app/(main)/admin/content/actions";
...
function SubmitButton() {
    ...
    return <Button disabled={pending}>{pending ? "Saving..." : "Save Favorites Page"}</Button>;
}
...
export function FavoritesPageForm({ config, siteConfig }: { config: any; siteConfig: any }) {
    // defaults
    const defaults = {
        title: "My Favorites",
        emptyTitle: "No favorites yet",
        emptyBody: "Start exploring properties and tap the heart icon to save them here for easy access.",
        headerStyle: "solid",
        heroImage: ""
    };
    ...
    const [state, formAction] = useActionState(async (prevState: any, formData: FormData) => {
        const result = await updateFavoritesConfig(prevState, formData);
        if (result?.message) {
    ...
    return (
        <div className="space-y-6 max-w-2xl">
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold">Edit Favorites Page</h1>
                    <p className="text-sm text-muted-foreground">Customize the layout and text of the public favorites page.</p>
                </div>
            </div>
            ...
            <form action={formAction} className="space-y-8">
                ...
                <div className="space-y-4 border p-4 rounded-md">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input name="title" defaultValue={current.title} placeholder="My Favorites" />
                    </div>
                ...
                <div className="space-y-4 border p-4 rounded-md">
                    ...
                    <p className="text-sm text-muted-foreground">Shown when the user has no favorite properties.</p>

                    <div className="space-y-2">
                        <Label>Title</Label>
                        <Input name="emptyTitle" defaultValue={current.emptyTitle} placeholder="No favorites yet" />
                    </div>

                    <div className="space-y-2">
                        <Label>Body Text</Label>
                        <Textarea name="emptyBody" defaultValue={current.emptyBody} placeholder="Start exploring..." rows={3} />
                    </div>
                </div>
