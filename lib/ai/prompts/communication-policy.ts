...
export function buildDealProtectiveCommunicationContract(input: BuildCommunicationContractInput = {}): string {
    ...
    return [
        ...
        "- Precision: Use probability or uncertainty phrasing only when the context actually contains uncertainty, incomplete authority, evolving status, or missing confirmation. Do not add hedge phrases by default when the facts are already clear.",
        ...
        "- URLs: When including a URL, place it on its own line and do not add trailing punctuation (especially a final period), so the link stays clickable in chat clients.",
        ...
        "- User wording: If the agent's instruction already specifies the intended wording or reads like a send-ready draft, preserve that meaning and phrasing style unless a change is required for factual accuracy, policy safety, or basic grammar.",
        "",
        "Use these phrasing patterns only when the context genuinely requires them:",
        '- "from what I know"',
        '- "at this stage"',
...
export function buildConversationalMessagingContract(input: BuildConversationalMessagingContractInput = {}): string {
    ...
    return [
        ...
        "- Use a mobile-first shape: hook, key facts, light context or benefit if useful, then one clear CTA.",
        ...
        "- Avoid bulky blocks of text. Put one idea, question, or fact per short line when writing for chat.",
        ...
        "- Avoid repeating the same opener, CTA, or phrase pattern from recent outbound messages. If the thread already has similar listing outreach, vary the shape and make the new message feel like a continuation.",
        '- Prefer simple, clear English over idioms, especially for WhatsApp/SMS. Avoid vague phrases such as "fancy a look?", "worth a look?", or "caught my eye" unless the contact already uses that style.',
        ...
        "- For property/listing messages, state facts first and benefits lightly; avoid brochure language, hard-sell framing, and over-explaining why the client should care.",
        "- Use emojis sparingly. Include at most one simple emoji only if it matches the contact's tone and channel.",
        "- Keep URLs on their own line after a blank line when possible, with no trailing punctuation.",
