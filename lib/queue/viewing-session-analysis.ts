...
import { runViewingSessionMessageAnalysis } from "@/lib/viewings/sessions/analysis";
...
const QUEUE_NAME = "viewing-session-analysis";
...
type ViewingSessionAnalysisJobData = {
    sessionId: string;
    messageId: string;
    force: boolean;
    queuedAt: string;
...
async function getQueueInstance() {
    if (!_queuePromise) {
        _queuePromise = (async () => {
            ...
            return new Queue<ViewingSessionAnalysisJobData>(QUEUE_NAME, {
...
export async function enqueueViewingSessionAnalysis(
    ...
    if (!input.force && message.analysisStatus === "completed" && message.translatedText) {
        return {
            accepted: false,
            mode: "skipped",
    ...
    const jobId = buildQueueJobId("viewing-session-analysis", messageId);
    ...
    try {
        ...
        await queue.add(
            "analyze-viewing-session-message",
            {
                sessionId,
                messageId,
                force: !!input.force,
        ...
        if (!allowInlineFallback) {
            return {
                ...
                reason: "enqueue_failed",
                jobId,
                error: String(queueError?.message || "Failed to enqueue viewing session analysis."),
            };
        }

        console.warn("[Queue] Failed to enqueue viewing session analysis. Falling back to inline processing:", queueError);
        void runViewingSessionMessageAnalysis({ sessionId, messageId }).catch((inlineErr) => {
            console.error("[Queue] Inline viewing session analysis fallback failed:", inlineErr);
        });
...
export async function initViewingSessionAnalysisWorker() {
    ...
    _workerPromise = (async () => {
        ...
        const worker = new Worker<ViewingSessionAnalysisJobData>(
            QUEUE_NAME,
            async (job: any) => {
                await runViewingSessionMessageAnalysis({
        ...
        worker.on("ready", () => {
            console.log("[Queue] Viewing session analysis worker is ready.");
        });

        worker.on("failed", (job: any, err: Error) => {
            console.error(`[Queue] Viewing session analysis job failed (${job?.id || "unknown"}): ${err.message}`);
        });
